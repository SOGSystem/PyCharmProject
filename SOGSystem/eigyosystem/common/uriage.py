...
from eigyosystem.common import as400connection


def uriagesql(form):
    datestart = time.strftime('%Y%m%d', form.cleaned_data['datestart'].timetuple())
    dateend = time.strftime('%Y%m%d', form.cleaned_data['dateend'].timetuple())
    userid = form.cleaned_data['userid']
    companyid = form.cleaned_data['companyid']

    wherestr = ' AND U.UTRN09 BETWEEN ' + datestart + ' AND ' + dateend

    if userid.strip() != '':
        print('123321  :' + userid.strip())
        wherestr = wherestr + ' AND U.UTRN16 = \'' + userid.strip() + '\''

    if companyid.strip() != '':
        wherestr = wherestr + ' AND K.TCORD1 = \'' + companyid.strip() + '\''

    # wherestr = wherestr + ' AND U.UTRN16 = ' + '69'

    dataresult = as400connection.as400connection()
    dataresult.execute('''SELECT
      CASE
        WHEN U.UTRN15 = '50'
        THEN '4.OS'
        WHEN
